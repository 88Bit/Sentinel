...
public class GatewayFlowRuleController {
    ...
    private DynamicRulePublisher<List<GatewayFlowRule>> rulePublisher;
...
//            repository.saveAll(rules);
            List<GatewayFlowRule> rules = ruleProvider.getRules(app);
            List<GatewayFlowRuleEntity> ruleEntities = rules.stream().map(s -> fromGatewayFlowRule(app, ip, port, s)).collect(Collectors.toList());
            repository.saveAll(ruleEntities);
